...
func (reg *etcdv3Registry) RegisterService(ctx context.Context, info *Instance) error {
...
func (reg *etcdv3Registry) UnregisterService(ctx context.Context, info *Instance) error {
	return reg.unregister(ctx, reg.registerKey(info))
}

// ListServices list service registered in registry with name `name`
func (reg *etcdv3Registry) ListServices(ctx context.Context, name string, scheme string) (services []*Instance, err error) {
...
func (reg *etcdv3Registry) WatchServices(ctx context.Context, name string, scheme string) (chan Endpoints, error) {
...
func (reg *etcdv3Registry) registerMetric(ctx context.Context, info *Instance) error {
	if info.Kind != ServiceGovernor {
		return nil
	}
...
func (reg *etcdv3Registry) registerBiz(ctx context.Context, info *Instance) error {
...
func (reg *etcdv3Registry) registerKey(info *Instance) string {
	return GetServiceKey(reg.Prefix, info)
}

func (reg *etcdv3Registry) registerValue(info *Instance) string {
	return GetServiceValue(info)
}

func deleteAddrList(al *Endpoints, prefix, scheme string, kvs ...*mvccpb.KeyValue) {
	for _, kv := range kvs {
...
func updateAddrList(al *Endpoints, prefix, scheme string, kvs ...*mvccpb.KeyValue) {
	for _, kv := range kvs {
		var addr = strings.TrimPrefix(string(kv.Key), prefix)
		switch {
		// 解析服务注册键
